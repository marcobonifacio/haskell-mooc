...
visit maze place =
  get >>= \x -> if place `elem` x then put x else void (put (place : x)) >>
  case lookup place maze of Nothing -> return ()
                            Just [] -> return ()
                            Just xs -> visit' maze xs where
                              visit' m [] = return ()
                              visit' m (x:xs) = visit m x >> visit' m xs

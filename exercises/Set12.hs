...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode val left) = LNode (f val) (fmap f left)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode val1 val2 list) = TwoNode (f val1) (f val2) (fmap f list)
...
instance Foldable List where
  foldr f ival Empty = ival
  foldr f ival (LNode val node) = f val (foldr f ival node)
...
instance Foldable TwoList where
  foldr f ival TwoEmpty = ival
  foldr f ival (TwoNode val1 val2 node) = f val1 (f val2 (foldr f ival node))
